      ...
      superagent
        ...
        .end( (err, _res) => {
          ...
          article.images = _res.body.items.map( img => {
            ...
            img.thumb = imageURL + 'thumbnails/' + img.basename.replace('image', 'thumb');
            return img;
          });
...
const buildArticles = req => {
  ...
  //Update from response the Hidden Articles
  if (cfg.hidden && cfg.hidden.length){
    req.articles.forEach( (a, i) => {
      if (cfg.hidden.indexOf(+a.number) > -1){
        req.articles[i] = {
          number: a.number,
          title: a.title,
          disabled: true
  ...
  //remove analysis articles for publishers
